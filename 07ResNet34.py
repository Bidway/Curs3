...
# Модифицированные блоки для ResNet34
def conv_block(x, filters, kernel_size=3, strides=1):
    f1, f2 = filters  # Для ResNet34 используем только 2 слоя в блоке

    shortcut = x

    x = layers.Conv2D(f1, (kernel_size, kernel_size), strides=strides, padding='same')(x)
    ...
    shortcut = layers.Conv2D(f2, (1, 1), strides=strides, padding='same')(shortcut)
...
def identity_block(x, filters, kernel_size=3):
    ...
    x = layers.Conv2D(f1, (kernel_size, kernel_size), padding='same')(x)
    x = layers.BatchNormalization()(x)
    x = layers.ReLU()(x)
...
def build_resnet34(input_shape, num_classes):
    print("Building ResNet34 manually...")
    ...
    # Conv2_x (3 блока)
    x = conv_block(x, [64, 64], strides=1)
    x = identity_block(x, [64, 64])
    x = identity_block(x, [64, 64])

    # Conv3_x (4 блока)
    x = conv_block(x, [128, 128], strides=2)
    x = identity_block(x, [128, 128])
    x = identity_block(x, [128, 128])
    x = identity_block(x, [128, 128])

    # Conv4_x (6 блоков)
    x = conv_block(x, [256, 256], strides=2)
    for _ in range(5):
        x = identity_block(x, [256, 256])

    # Conv5_x (3 блока)
    x = conv_block(x, [512, 512], strides=2)
...
def save_model(model):
    print("Saving model...")
    dirname = f"saved_models/{datetime.now().strftime('%Y%m%d-%H%M%S')}-resnet34-manual"
...
def main():
    ...
    with mlflow.start_run(run_name="resnet34-manual") as run:
        mlflow.log_param("architecture", "Manual ResNet34")
        ...
        model = build_resnet34((IMG_HEIGHT, IMG_WIDTH, 3), N_LABELS)
